...
public class SurveyServices {
        ...
        public ByteArrayResource createExcel(ExcelQueryObject excelQueryObject) {
                try {
                        Survey survey = surveyRepo.findById(excelQueryObject.getSurveyId())
                                        .orElseThrow(() -> new NotFoundException("Survey with id "
                                                        + excelQueryObject.getSurveyId() + " not found"));
                        List<Response> responses;
                        if (excelQueryObject.getVillage() != null) {
                                Village village = villageRepo.findById(excelQueryObject.getVillage())
                                                .orElseThrow(() -> new NotFoundException("Village with id "
                                                                + excelQueryObject.getVillage() + " not found"));

                                responses = responseRepo.findAllBySurveyAndVillageAndYear(
                                                survey,
                                                village,
                                                excelQueryObject.getYear()).stream()
                        ...
                        for (Response response : responses) {
                                ...
                                excelFieldObjects.add(
                                                set("surveyId", response.getId(),
                                                                List.of(response.getResponseId().getSurveyId())));
                                excelFieldObjects.add(set("date", response.getId(),
                                                List.of(response.getDate().toString().substring(0, 10))));
                                excelFieldObjects.add(set("username", response.getId(),
                                                List.of(response.getUser().getUserName())));
                                excelFieldObjects.add(set("villageName", response.getId(),
                                                List.of(village.getVillageName())));
                                excelFieldObjects.add(set("villageCode", response.getId(),
                                                List.of(village.getVillageCode())));
                                excelFieldObjects.add(
                                                set("blockName", response.getId(), List.of(village.getBlockName())));
                                excelFieldObjects.add(
                                                set("blockCode", response.getId(), List.of(village.getBlockCode())));
                                excelFieldObjects.add(set("state", response.getId(), List.of(village.getState())));
                                excelFieldObjects
                                                .add(set("district", response.getId(), List.of(village.getDistrict())));
                                if (survey.getSurveyName().contains("House")) {
                                        excelFieldObjects.add(set("gramPanchayatName", response.getId(),
                                                        List.of(response.getResponseId().getGramPanchayatName())));
                                        System.out.println("response.getResponseId().getGramPanchayatCode() = "
                                                        + response.getResponseId().getGramPanchayatCode());
                                        excelFieldObjects.add(set("gramPanchayatCode", response.getId(), List.of(String
                                                        .valueOf(response.getResponseId().getGramPanchayatCode()))));
                                        excelFieldObjects.add(set("ward", response.getId(),
                                                        List.of(String.valueOf(response.getResponseId().getWardNo()))));
                                        excelFieldObjects.add(set("headName", response.getId(),
                                                        List.of(response.getResponseId().getHeadName())));
                                        excelFieldObjects.add(set("headAadhaarNumber", response.getId(),
                                                        List.of(response.getResponseId().getAadharNo())));
                                        excelFieldObjects.add(set("headMobileNumber", response.getId(),
                                                        List.of(response.getResponseId().getMobileNo())));
                                        excelFieldObjects.add(set("rationCardNumber", response.getId(),
                                                        List.of(response.getResponseId().getRationNo())));
                                        excelFieldObjects.add(set("rationCardType", response.getId(),
                                                        List.of(response.getResponseId().getCardType())));
                                }
                                ...
                                for (ResponseRecord responseRecord : response.getResponseRecords()) {
                                        ...
                                        excelFieldObject.setAnswers(responseRecord.getAnswers().stream()
                                                        .map(AnswerOption::getOptionName).toList());
